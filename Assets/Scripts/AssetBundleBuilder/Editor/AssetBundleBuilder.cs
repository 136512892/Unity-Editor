...
{
    [MenuItem("Example/Resource/AssetBundle Builder")]
    ...
    private void OnEnable()
    {
        ...
        string dataPath = Path.GetFullPath(".")
            .Replace("\\", "/") + "/Library/AssetBundleBuild.dat";
        //判断数据文件是否存在
    ...
    private void OnGUI()
    {
        ...
        //目标平台
        data.buildTarget = (BuildTarget)EditorGUILayout
            .EnumPopup("Build Target", data.buildTarget);
        GUILayout.BeginHorizontal();
        //输出路径
        data.outputPath = EditorGUILayout
            .TextField("Output Path", data.outputPath);
        //浏览按钮
        if (GUILayout.Button("Browse", GUILayout.Width(60f)))
        {
            //选择输出路径
            string selectedPath = EditorUtility.OpenFolderPanel(
                "AssetsBundle构建路径", Application.dataPath, string.Empty);
        ...
        //压缩方式
        data.compressionType = (BuildTabData.CompressionType)
            EditorGUILayout.EnumPopup("Compression", data.compressionType);
        GUILayout.Space(20f);
        //是否拷贝至Assets/StreamingAssets
        data.copy2StreamingAssets = GUILayout.Toggle(
            data.copy2StreamingAssets, GUIContents.copy2StreamingAssets);
        EditorGUILayout.Space();
        //Options
        ...
        data.disableWriteTypeTree = GUILayout.Toggle(
            data.disableWriteTypeTree, 
            GUIContents.disableWriteTypeTree);
        data.forceRebuildAssetBundle = GUILayout.Toggle(
            data.forceRebuildAssetBundle, 
            GUIContents.forceRebuildAssetBundle);
        data.ignoreTypeTreeChanges = GUILayout.Toggle(
            data.ignoreTypeTreeChanges,
            GUIContents.ignoreTypeTreeChanges);
        data.appendHashToAssetBundleName = GUILayout.Toggle(
            data.appendHashToAssetBundleName, 
            GUIContents.appendHash);
        data.strictMode = GUILayout.Toggle(
            data.strictMode,
            GUIContents.strictMode);
        data.dryRunBuild = GUILayout.Toggle(
            data.dryRunBuild, 
            GUIContents.dryRunBuild);
    ...
    internal void CopyDirectory(string sourceDir, string destDir)
    {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            ...
            string fileDirName = Path
                .GetDirectoryName(filePath).Replace("\\", "/");
            string fileName = Path.GetFileName(filePath);
            //路径拼接
            string newFilePath = Path.Combine(
                fileDirName.Replace(sourceDir, destDir), fileName);
            //拷贝文件
    ...
    private void BuildAssetBundle()
    {
        ...
        {
            Debug.Log(string.Format("路径不存在，进行创建 {0}",
                data.outputPath));
        ...
        if (data.disableWriteTypeTree) 
            options |= BuildAssetBundleOptions.DisableWriteTypeTree;
        if (data.forceRebuildAssetBundle)
            options |= BuildAssetBundleOptions.ForceRebuildAssetBundle;
        if (data.ignoreTypeTreeChanges) 
            options |= BuildAssetBundleOptions.IgnoreTypeTreeChanges;
        if (data.appendHashToAssetBundleName)
        ...
        AssetBundleManifest manifest = BuildPipeline
            .BuildAssetBundles(data.outputPath, options, data.buildTarget);
        //Map
        ...
        if (data.copy2StreamingAssets)
        {
            ...
            if (!data.outputPath.StartsWith(
                Application.streamingAssetsPath))
            {
                CopyDirectory(data.outputPath,
                    Application.streamingAssetsPath);
            }
        }
    ...
    private class GUIContents
    {
        public static GUIContent copy2StreamingAssets = new GUIContent(
            ...
            "Copy asset bundle to Assets/StreamingAssets after " +
            "build completed for use in simulation mode.");
        ...
        public static GUIContent ignoreTypeTreeChanges = new GUIContent(
            ...
            "Ignore the type tree changes when " +
            "doing the incremental build check.");
        public static GUIContent appendHash = new GUIContent(
            "Append Hash",
            "Append the hash to to asset bundle name.");
        public static GUIContent strictMode = new GUIContent(
            "Strict Mode",
            " Do not allow the build to succeed if " +
            "any errors are reporting during it.");
