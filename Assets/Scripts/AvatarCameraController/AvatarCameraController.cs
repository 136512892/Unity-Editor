...
public class AvatarCameraController : MonoBehaviour
{
    ...
    //控制模式 默认第三人称
    [SerializeField] 
    ControlMode controlMode = ControlMode.ThirdPersonControl;
    //切换控制模式的快捷键
    [SerializeField] 
    private KeyCode modeChangeKey;
    //视角前方是否与Avatar对齐
    [SerializeField] 
    private bool forwardAlignWithAvatar;
    //水平方向灵敏度
    ...
    private float rotYMinLimit = -40f;
    //旋转y值的最大值限制
    [SerializeField, Range(10f, 80f)] 
    private float rotYMaxLimit = 70f;
    //插值到目标旋转值所需的时间
    ...
    //第一人称模式所用的固定距离
    [SerializeField, Range(-1.5f, 0f)] 
    private float fpmDistance = -.5f;
    //鼠标滚轮灵敏度
    [SerializeField, Range(1f, 5f)] 
    ...
    //与Avatar在水平方向上的偏移值
    //仅在forwardAlignWithAvatar为true时开启使用
    ...
    [SerializeField]
    private bool ctrlAvatarRotWhenFPMode = true;

    private void Start()
    {
        targetDistance = Mathf.Clamp(distance, 
    ...
    private void LateUpdate()
    {
        ...
        {
            horizontal = forwardAlignWithAvatar ? 0f : Input.GetAxis("Mouse X") 
                * Time.deltaTime* 100f * horizontalSensitivity;
            vertical = Input.GetAxis("Mouse Y") 
                * Time.deltaTime * 100f * verticalSensitivity;
        ...
        distance -= Input.GetAxis("Mouse ScrollWheel") * Time.deltaTime 
            * 100f * scollSensitivity * (invertScrollDirection ? -1f : 1f);
        //距离钳制
        distance = Mathf.Clamp(distance, minDistanceLimit, 
            maxDistanceLimit);
        //插值方式计算距离
        targetDistance = controlMode == ControlMode.ThirdPersonControl
            ? Mathf.Lerp(targetDistance, distance, Time.deltaTime
                * scollSensitivity) : fpmDistance;

        //目标旋转值
        Quaternion targetRotation = Quaternion.Euler(rotY, rotX, 0f);
        //旋转值插值率
        float rotationLerpPct = 1f - Mathf.Exp(Mathf.Log(1f - .99f) 
            / rotationLerpTime * Time.deltaTime);
        //插值方式计算旋转值
        targetRotation = Quaternion.Lerp(transform.rotation, 
            targetRotation, rotationLerpPct);

        //目标坐标值
        Vector3 targetPosition = targetRotation * Vector3.forward
            * -targetDistance + avatar.position + Vector3.up * height;
        //避障
        targetPosition = ObstacleAvoidance(targetPosition,
            avatar.position + Vector3.up * height, .1f, distance);

        transform.position = targetPosition + Vector3.left
            * horizontalOffset;
        transform.rotation = targetRotation;

        //第一人称控制模式 相机视角旋转的同时控制Avatar角色的旋转
        if (controlMode == ControlMode.FirstPersonControl 
            && ctrlAvatarRotWhenFPMode)
        {
    ...
    private void OnDrawGizmos()
    {
        ...
        Ray ray = new Ray(target, targetPosition - target);
            ...
            distance, obstacleLayer))
        {
            ...
            Vector3 pos = ray.GetPoint(hit.distance - .2f);
